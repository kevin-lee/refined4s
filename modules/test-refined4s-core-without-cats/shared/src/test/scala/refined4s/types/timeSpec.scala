...
object timeSpec extends Properties {
  ...
  object MonthSpec {
    def tests: List[Test] = List(
      example("test Month(1 ... 12) should compile", testApplyValid),
      example("test Month(invalid month number) should not compile", testApplyInvalid),
      property("test Month.from(valid month number) should return Either[String, Month] = Right(Month)", testFromValid),
      property("test Month.from(invalid month number) should return Either[String, Month] = Left(error message)", testFromInvalid),
      property("test Month.unsafeFrom(valid month number) should return Month", testUnsafeFromValid),
      property("test Month.unsafeFrom(invalid month number) should throw IllegalArgumentException", testUnsafeFromInvalid),
      property("test Month.value should return Int", testValue),
      property("test Month.unapplyWithPatternMatching should return Int", testUnapplyWithPatternMatching),
      property("test Month.Ordering should return right order Int", testOrdering),
      property("test Month.NumericOrdered should return right order Int", testNumericOrdered),
      example("test Month.MinValue should return Month(1)", testMinValue),
      example("test Month.MaxValue should return Month(12)", testMaxValue),
    )
    ...
    def testApplyInvalid: Result = {
      ...
      val compilationResult2 = typeCheckErrors(
        """
          import refined4s.types.time.*
          Month(0)
        """
      ...
      val compilationResult3 = typeCheckErrors(
        """
          import refined4s.types.time.*
          Month(13)
        """
      ...
      val actual1 = compilationResult1.map(_.message).mkString
      val actual2 = compilationResult2.map(_.message).mkString
      val actual3 = compilationResult3.map(_.message).mkString
    ...
    def testFromValid: Property = for {
      month <- TimeGens.genMonth.log("month")
    } yield {
      val expected = Right(month)
      val actual   = Month.from(month.value)
    ...
    def testUnsafeFromValid: Property = for {
      month <- TimeGens.genMonth.log("month")
    } yield {
      val expected = month
      val actual   = Month.unsafeFrom(month.value)
      ...
      Result.all(
        List(
          actual
            ...
            .log("The thrown exception doesn't match. It should be IllegalArgumentException"),
          (actual.left.map(_.getMessage) ==== Left(expected)).log("Error message doesn't match"),
        )
      )
    }
    ...
    def testValue: Property =
      for {
        n <- Gen.int(Range.linear(1, 12)).log("n")
      } yield {
        val expected = n
        val actual   = Month.unsafeFrom(n)
    ...
    def testOrdering: Property =
      for {
        month1 <- TimeGens.genMonth.log("month1")
        month2 <- TimeGens.genMonth.log("month2")
      } yield {
        Ordering[Month].compare(month1, month2) ==== Ordering[Int].compare(month1.value, month2.value)
      }
    ...
    def testNumericOrdered: Property =
      for {
        ...
        val input1: Ordered[Month] = month1
        val input2: Month          = month2
        input1.compare(input2) ==== month1.value.compare(month2.value)
      }

    @SuppressWarnings(Array("org.wartremover.warts.ToString"))
    def testMinValue: Result = {
      val expected  = Month(1)
      val actual    = Month.MinValue
      val actualMin = Month.min

      Result.all(
        List(
          (actual ==== expected).log(s"Month.MinValue ==== ${expected.toString} failed"),
          (actualMin ==== expected).log(s"Month.min ==== ${expected.toString} failed"),
        )
      )
    }
    ...
    def testMaxValue: Result = {
      val expected  = Month(12)
      val actual    = Month.MaxValue
      val actualMax = Month.max

      Result.all(
        List(
          (actual ==== expected).log(s"Month.MaxValue ==== ${expected.toString} failed"),
          (actualMax ==== expected).log(s"Month.max ==== ${expected.toString} failed"),
        )
      )
    }
  }
  ...
  object DaySpec {
    def tests: List[Test] = List(
      example("test Day(1 ... 31) should compile", testApplyValid),
      example("test Day(invalid day number) should not compile", testApplyInvalid),
      property("test Day.from(valid day number) should return Either[String, Day] = Right(Day)", testFromValid),
      property("test Day.from(invalid day number) should return Either[String, Day] = Left(error message)", testFromInvalid),
      property("test Day.unsafeFrom(valid day number) should return Day", testUnsafeFromValid),
      property("test Day.unsafeFrom(invalid day number) should throw IllegalArgumentException", testUnsafeFromInvalid),
      property("test Day.value should return Int", testValue),
      property("test Day.unapplyWithPatternMatching should return Int", testUnapplyWithPatternMatching),
      property("test Day.Ordering should return right order Int", testOrdering),
      property("test Day.NumericOrdered should return right order Int", testNumericOrdered),
      example("test Day.MinValue should return Day(1)", testMinValue),
      example("test Day.MaxValue should return Day(31)", testMaxValue),
    )
    ...
    def testApplyInvalid: Result = {
      ...
      Result.all(
        List(
          Result.assert(shouldNotCompile1).log("""Day(-1) should have failed compilation but it succeeded."""),
          Result.assert(shouldNotCompile2).log("""Day(0) should have failed compilation but it succeeded."""),
          Result.assert(shouldNotCompile3).log("""Day(32) should have failed compilation but it succeeded."""),
        )
      )
    }
    ...
    def testFromValid: Property = for {
      ...
      val expected = Right(day)
      val actual   = Day.from(day.value)
    ...
    def testUnsafeFromValid: Property = for {
      ...
      val expected = day
      val actual   = Day.unsafeFrom(day.value)
    ...
    def testValue: Property =
      for {
        n <- Gen.int(Range.linear(1, 31)).log("n")
      } yield {
        val expected = n
        val actual   = Day.unsafeFrom(n)
        actual.value ==== expected
      }
    ...
    def testOrdering: Property =
      for {
        day1 <- TimeGens.genDay.log("day1")
        day2 <- TimeGens.genDay.log("day2")
      } yield {
        Ordering[Day].compare(day1, day2) ==== Ordering[Int].compare(day1.value, day2.value)
      }
    ...
    def testNumericOrdered: Property =
      for {
        ...
        val input1: Ordered[Day] = day1
        val input2: Day          = day2
        input1.compare(input2) ==== day1.value.compare(day2.value)
      }

    @SuppressWarnings(Array("org.wartremover.warts.ToString"))
    def testMinValue: Result = {
      val expected  = Day(1)
      val actual    = Day.MinValue
      val actualMin = Day.min

      Result.all(
        List(
          (actual ==== expected).log(s"Day.MinValue ==== ${expected.toString} failed"),
          (actualMin ==== expected).log(s"Day.min ==== ${expected.toString} failed"),
        )
      )
    }
    ...
    def testMaxValue: Result = {
      val expected  = Day(31)
      val actual    = Day.MaxValue
      val actualMax = Day.max

      Result.all(
        List(
          (actual ==== expected).log(s"Day.MaxValue ==== ${expected.toString} failed"),
          (actualMax ==== expected).log(s"Day.max ==== ${expected.toString} failed"),
        )
      )
    }
  ...
  object HourSpec {
    def tests: List[Test] = List(
      example("test Hour(0 ... 23) should compile", testApplyValid),
      example("test Hour(invalid hour number) should not compile", testApplyInvalid),
      property("test Hour.from(valid hour number) should return Either[String, Hour] = Right(Hour)", testFromValid),
      property("test Hour.from(invalid hour number) should return Either[String, Hour] = Left(error message)", testFromInvalid),
      property("test Hour.unsafeFrom(valid hour number) should return Hour", testUnsafeFromValid),
      property("test Hour.unsafeFrom(invalid hour number) should throw IllegalArgumentException", testUnsafeFromInvalid),
      property("test Hour.value should return Int", testValue),
      property("test Hour.unapplyWithPatternMatching should return Int", testUnapplyWithPatternMatching),
      property("test Hour.Ordering should return right order Int", testOrdering),
      property("test Hour.NumericOrdered should return right order Int", testNumericOrdered),
      example("test Hour.MinValue should return Hour(0)", testMinValue),
      example("test Hour.MaxValue should return Hour(23)", testMaxValue),
    )
    ...
    def testApplyInvalid: Result = {
      ...
      Result.all(
        List(
          Result.assert(shouldNotCompile1).log("""Hour(-1) should have failed compilation but it succeeded."""),
          Result.assert(shouldNotCompile2).log("""Hour(24) should have failed compilation but it succeeded."""),
        )
      )
    }
    ...
    def testFromValid: Property = for {
      ...
      val expected = Right(hour)
      val actual   = Hour.from(hour.value)
    ...
    def testUnsafeFromValid: Property = for {
      ...
      val expected = hour
      val actual   = Hour.unsafeFrom(hour.value)
    ...
    def testValue: Property =
      for {
        n <- Gen.int(Range.linear(0, 23)).log("n")
      } yield {
        val expected = n
        val actual   = Hour.unsafeFrom(n)
        actual.value ==== expected
      }
    ...
    def testOrdering: Property =
      for {
        hour1 <- TimeGens.genHour.log("hour1")
        hour2 <- TimeGens.genHour.log("hour2")
      } yield {
        Ordering[Hour].compare(hour1, hour2) ==== Ordering[Int].compare(hour1.value, hour2.value)
      }
    ...
    def testNumericOrdered: Property =
      for {
        ...
        val input1: Ordered[Hour] = hour1
        val input2: Hour          = hour2
        input1.compare(input2) ==== hour1.value.compare(hour2.value)
      }

    @SuppressWarnings(Array("org.wartremover.warts.ToString"))
    def testMinValue: Result = {
      val expected  = Hour(0)
      val actual    = Hour.MinValue
      val actualMin = Hour.min

      Result.all(
        List(
          (actual ==== expected).log(s"Hour.MinValue ==== ${expected.toString} failed"),
          (actualMin ==== expected).log(s"Hour.min ==== ${expected.toString} failed"),
        )
      )
    }
    ...
    @SuppressWarnings(Array("org.wartremover.warts.ToString"))
    def testMaxValue: Result = {
      val expected  = Hour(23)
      val actual    = Hour.MaxValue
      val actualMax = Hour.max

      Result.all(
        List(
          (actual ==== expected).log(s"Hour.MaxValue ==== ${expected.toString} failed"),
          (actualMax ==== expected).log(s"Hour.max ==== ${expected.toString} failed"),
        )
      )
    }
  ...
  object MinuteSpec {
    def tests: List[Test] = List(
      example("test Minute(0 ... 59) should compile", testApplyValid),
      example("test Minute(invalid minute number) should not compile", testApplyInvalid),
      property("test Minute.from(valid minute number) should return Either[String, Minute] = Right(Minute)", testFromValid),
      property("test Minute.from(invalid minute number) should return Either[String, Minute] = Left(error message)", testFromInvalid),
      property("test Minute.unsafeFrom(valid minute number) should return Minute", testUnsafeFromValid),
      property("test Minute.unsafeFrom(invalid minute number) should throw IllegalArgumentException", testUnsafeFromInvalid),
      property("test Minute.value should return Int", testValue),
      property("test Minute.unapplyWithPatternMatching should return Int", testUnapplyWithPatternMatching),
      property("test Minute.Ordering should return right order Int", testOrdering),
      property("test Minute.NumericOrdered should return right order Int", testNumericOrdered),
      example("test Minute.MinValue should return Minute(0)", testMinValue),
      example("test Minute.MaxValue should return Minute(59)", testMaxValue),
    )
    ...
    def testApplyInvalid: Result = {
      ...
      Result.all(
        List(
          Result.assert(shouldNotCompile1).log("""Minute(-1) should have failed compilation but it succeeded."""),
          Result.assert(shouldNotCompile2).log("""Minute(60) should have failed compilation but it succeeded."""),
        )
      )
    }
    ...
    def testFromValid: Property = for {
      ...
      val expected = Right(minute)
      val actual   = Minute.from(minute.value)
    ...
    def testUnsafeFromValid: Property = for {
      ...
      val expected = minute
      val actual   = Minute.unsafeFrom(minute.value)
    ...
    def testValue: Property = for {
      n <- Gen.int(Range.linear(0, 59)).log("n")
    } yield {
      val expected = n
      val actual   = Minute.unsafeFrom(n)
      actual.value ==== expected
    }
    ...
    def testOrdering: Property =
      for {
        minute1 <- TimeGens.genMinute.log("minute1")
        minute2 <- TimeGens.genMinute.log("minute2")
      } yield {
        Ordering[Minute].compare(minute1, minute2) ==== Ordering[Int].compare(minute1.value, minute2.value)
      }
    ...
    def testNumericOrdered: Property =
      for {
        ...
        val input1: Ordered[Minute] = minute1
        val input2: Minute          = minute2
        input1.compare(input2) ==== minute1.value.compare(minute2.value)
      }

    @SuppressWarnings(Array("org.wartremover.warts.ToString"))
    def testMinValue: Result = {
      val expected  = Minute(0)
      val actual    = Minute.MinValue
      val actualMin = Minute.min

      Result.all(
        List(
          (actual ==== expected).log(s"Minute.MinValue ==== ${expected.toString} failed"),
          (actualMin ==== expected).log(s"Minute.min ==== ${expected.toString} failed"),
        )
      )
    }
    ...
    @SuppressWarnings(Array("org.wartremover.warts.ToString"))
    def testMaxValue: Result = {
      val expected  = Minute(59)
      val actual    = Minute.MaxValue
      val actualMax = Minute.max

      Result.all(
        List(
          (actual ==== expected).log(s"Minute.MaxValue ==== ${expected.toString} failed"),
          (actualMax ==== expected).log(s"Minute.max ==== ${expected.toString} failed"),
        )
      )
    }
  ...
  object SecondSpec {
    def tests: List[Test] = List(
      example("test Second(0 ... 59) should compile", testApplyValid),
      example("test Second(invalid second number) should not compile", testApplyInvalid),
      property("test Second.from(valid second number) should return Either[String, Second] = Right(Second)", testFromValid),
      property("test Second.from(invalid second number) should return Either[String, Second] = Left(error message)", testFromInvalid),
      property("test Second.unsafeFrom(valid second number) should return Second", testUnsafeFromValid),
      property("test Second.unsafeFrom(invalid second number) should throw IllegalArgumentException", testUnsafeFromInvalid),
      property("test Second.value should return Int", testValue),
      property("test Second.unapplyWithPatternMatching should return Int", testUnapplyWithPatternMatching),
      property("test Second.Ordering should return right order Int", testOrdering),
      property("test Second.NumericOrdered should return right order Int", testNumericOrdered),
      example("test Second.MinValue should return Second(0)", testMinValue),
      example("test Second.MaxValue should return Second(59)", testMaxValue),
    )
    ...
    def testFromValid: Property = for {
      ...
      val expected = Right(second)
      val actual   = Second.from(second.value)
    ...
    def testUnsafeFromValid: Property = for {
      ...
      val expected = second
      val actual   = Second.unsafeFrom(second.value)
    ...
    def testValue: Property = for {
      ...
      val expected = n
      val actual   = Second.unsafeFrom(n)
      actual.value ==== expected
    }
    ...
    def testOrdering: Property =
      for {
        second1 <- TimeGens.genSecond.log("second1")
        second2 <- TimeGens.genSecond.log("second2")
      } yield {
        Ordering[Second].compare(second1, second2) ==== Ordering[Int].compare(second1.value, second2.value)
      }
    ...
    def testNumericOrdered: Property =
      for {
        ...
        val input1: Ordered[Second] = second1
        val input2: Second          = second2
        input1.compare(input2) ==== second1.value.compare(second2.value)
      }

    @SuppressWarnings(Array("org.wartremover.warts.ToString"))
    def testMinValue: Result = {
      val expected  = Second(0)
      val actual    = Second.MinValue
      val actualMin = Second.min

      Result.all(
        List(
          (actual ==== expected).log(s"Second.MinValue ==== ${expected.toString} failed"),
          (actualMin ==== expected).log(s"Second.min ==== ${expected.toString} failed"),
        )
      )
    }
    ...
    @SuppressWarnings(Array("org.wartremover.warts.ToString"))
    def testMaxValue: Result = {
      val expected  = Second(59)
      val actual    = Second.MaxValue
      val actualMax = Second.max

      Result.all(
        List(
          (actual ==== expected).log(s"Second.MaxValue ==== ${expected.toString} failed"),
          (actualMax ==== expected).log(s"Second.max ==== ${expected.toString} failed"),
        )
      )
    }
  ...
  object MillisSpec {
    def tests: List[Test] = List(
      example("test Millis(0 ... 999) should compile", testApplyValid),
      example("test Millis(invalid millisecond number) should not compile", testApplyInvalid),
      property("test Millis.from(valid millisecond number) should return Either[String, Millis] = Right(Millis)", testFromValid),
      property("test Millis.from(invalid millisecond number) should return Either[String, Millis] = Left(error message)", testFromInvalid),
      property("test Millis.unsafeFrom(valid millisecond number) should return Millis", testUnsafeFromValid),
      property("test Millis.unsafeFrom(invalid millisecond number) should throw IllegalArgumentException", testUnsafeFromInvalid),
      property("test Millis.value should return Int", testValue),
      property("test Millis.unapplyWithPatternMatching should return Int", testUnapplyWithPatternMatching),
      property("test Millis.Ordering should return right order Int", testOrdering),
      property("test Millis.NumericOrdered should return right order Int", testNumericOrdered),
      example("test Millis.MinValue should return Millis(0)", testMinValue),
      example("test Millis.MaxValue should return Millis(999)", testMaxValue),
    )
    ...
    def testFromValid: Property = for {
      ...
      val expected = Right(millis)
      val actual   = Millis.from(millis.value)
    ...
    def testUnsafeFromValid: Property = for {
      ...
      val expected = millis
      val actual   = Millis.unsafeFrom(millis.value)
      actual ==== expected
    }
    ...
    def testValue: Property = for {
      n <- Gen.int(Range.linear(0, 999)).log("n")
    } yield {
      val expected = n
      val actual   = Millis.unsafeFrom(n)
      actual.value ==== expected
    }
    ...
    def testOrdering: Property =
      for {
        millis1 <- TimeGens.genMillis.log("millis1")
        millis2 <- TimeGens.genMillis.log("millis2")
      } yield {
        Ordering[Millis].compare(millis1, millis2) ==== Ordering[Int].compare(millis1.value, millis2.value)
      }
    ...
    def testNumericOrdered: Property =
      for {
        ...
        val input1: Ordered[Millis] = millis1
        val input2: Millis          = millis2
        input1.compare(input2) ==== millis1.value.compare(millis2.value)
      }
